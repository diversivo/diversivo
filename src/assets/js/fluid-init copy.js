const canvas = document.getElementsByClassName('fluid-canvas')[0]; // Canvas
...
function supportRenderTextureFormat(wgl, internalFormat, format, type) {
  ...
  wgl.texImage2D(wgl.TEXTURE_2D, 0, internalFormat, 4, 4, 0, format, type, null);

  const fbo = wgl.createFramebuffer();
  wgl.bindFramebuffer(wgl.FRAMEBUFFER, fbo);
  wgl.framebufferTexture2D(wgl.FRAMEBUFFER, wgl.COLOR_ATTACHMENT0, wgl.TEXTURE_2D, texture, 0);

  const status = wgl.checkFramebufferStatus(wgl.FRAMEBUFFER);
  return status === wgl.FRAMEBUFFER_COMPLETE;
}
function getSupportedFormat(wgl, internalFormat, format, type) {
  if (!supportRenderTextureFormat(wgl, internalFormat, format, type)) {
    switch (internalFormat) {
      case wgl.R16F:
        return getSupportedFormat(wgl, wgl.RG16F, wgl.RG, type);
      case wgl.RG16F:
        return getSupportedFormat(wgl, wgl.RGBA16F, wgl.RGBA, type);
      default:
        return null;
    }
  }
...
function getWebGLContext(canvasEle) {
  const params = {
    alpha: true,
    depth: false,
    stencil: false,
  ...
  let wgl = canvasEle.getContext('webgl2', params);
  const isWebGL2 = !!wgl;
  if (!isWebGL2) {
    wgl = canvasEle.getContext('webgl', params) || canvasEle.getContext('experimental-webgl', params);
  }
  ...
  if (isWebGL2) {
    ...
    supportLinearFiltering = wgl.getExtension('OES_texture_float_linear');
  } else {
    halfFloat = wgl.getExtension('OES_texture_half_float');
    supportLinearFiltering = wgl.getExtension('OES_texture_half_float_linear');
  }

  wgl.clearColor(0.0, 0.0, 0.0, 1.0);

  const halfFloatTexType = isWebGL2 ? wgl.HALF_FLOAT : halfFloat.HALF_FLOAT_OES;
  let formatRGBA;
  let formatRG;
  let formatR;

  if (isWebGL2) {
    formatRGBA = getSupportedFormat(wgl, wgl.RGBA16F, wgl.RGBA, halfFloatTexType);
    formatRG = getSupportedFormat(wgl, wgl.RG16F, wgl.RG, halfFloatTexType);
    formatR = getSupportedFormat(wgl, wgl.R16F, wgl.RED, halfFloatTexType);
  }
  else {
    formatRGBA = getSupportedFormat(wgl, wgl.RGBA, wgl.RGBA, halfFloatTexType);
    formatRG = getSupportedFormat(wgl, wgl.RGBA, wgl.RGBA, halfFloatTexType);
    formatR = getSupportedFormat(wgl, wgl.RGBA, wgl.RGBA, halfFloatTexType);
  }
...
function compileShader(type, source, keywords) {
  const newSource = addKeywords(source, keywords); // POSIBLE ERROR!!!!!!
...
var ditheringTexture = createTextureAsync('https://cdn.diversivo.cl/fluid/LDR_LLL1_0.png');
...
function getResolution(resolution) {
  ...
  let response = { width: min, height: max };

  if (gl.drawingBufferWidth > gl.drawingBufferHeight) {
    response = { width: max, height: min };
  }
...
function resizeDoubleFBO(target, w, h, internalFormat, format, type, param) {
  ...
  if (newTarget.width === w && newTarget.height === h) {
    return newTarget;
  }
  newTarget.read = resizeFBO(newTarget.read, w, h, internalFormat, format, type, param);
  newTarget.write = createFBO(w, h, internalFormat, format, type, param);
  newTarget.width = w;
  newTarget.height = h;
  newTarget.texelSizeX = 1.0 / w;
  newTarget.texelSizeY = 1.0 / h;
  return target;
}
...
function correctRadius(radius) {
  ...
  if (aspectRatio > 1) newRadius *= aspectRatio;
  return newRadius;
}
...
var overlay = document.getElementsByClassName('fluid__content')[0];
